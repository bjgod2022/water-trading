...
klines[KLINE_MAP.Hour] = (stock) => {
    ...
    // https://ifzq.gtimg.cn/appstock/app/minute/query?_var=min_data_sh600103&code=sh600103&r=0.12174363108800135
    // let _api=`http://data.gtimg.cn/flashdata/hushen/minute/${stock.prefix}${stock.code}.js?maxage=110&rnd=${Math.random()}`
    // let _api=`http://data.gtimg.cn/flashdata/hushen/minute/${stock.prefix}${stock.code}.js?maxage=110&rnd=${Math.random()}`
    let _api = `https://ifzq.gtimg.cn/appstock/app/minute/query?_var=min_data_${stock.prefix}${stock.code}&code=${stock.prefix}${stock.code}&r=${Math.random()}`
    return new Promise((resolve, reject) => {
        appendScript(_api).then(() => {
            let _datas = window[`min_data_${_code}`].data[_code].data.data
            let _klines = []
            _datas.forEach(data => {
                // 过滤空字符串和日期
...
klines[KLINE_MAP.Day] = (stock) => {
    ...
    // let _code = `${stock.prefix}${stock.code}`
    let _code = `${stock.codes}`
    let _api = `http://web.ifzq.gtimg.cn/appstock/app/fqkline/get?_var=kline_dayqfq&param=${_code},day,,,770,qfq&r=${Math.random()}`
    return new Promise((resolve, reject) => {
        appendScript(_api).then(() => {

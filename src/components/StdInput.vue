<template>
    <span>
        <input v-if="type === 'text' || !type" :value="value" class="tvjs-std-input" :style="style" :placeholder="name"
            @change="$emit('change', $event.target.value)" @input="$emit('input', $event.target.value)">
        <select v-else-if="type === 'select'" class="tvjs-std-input" :style="style" :value="value"

...
export default {
  ...
  methods: {
    ...
    fetchHint(query) {
      this.$http
        .get(
          `http://smartbox.gtimg.cn/s3/?t=all&q=${query}&cb=zepto_suggest_1553825409638`
        )
        .then((res) => {
          let data = res.data.substring(8, res.data.length - 1).split("^");
          const canBeUse = ["sh", "sz"]; // 可以添加的市场
          // 将可用的存储起来
          let temp = [];
     ...
     */
    handleAction(code) {
      let mystorage = storage.get(this.dataset, "");
      if (mystorage !== null && mystorage !== "") {
        let splitCode = mystorage.split(","); // 获取存储的代码
        // 判断是否存在
        if (splitCode.indexOf(code) > -1 || stockIndex.indexOf(code) > -1) {
          ...
          storage.set(this.dataset, `${code},${mystorage}`); // 非空的情况
        ...
        storage.set(this.dataset, code); // 空的情况
        this.$toasted.show("添加完成", {
          theme: "toasted-primary",
          position: "bottom-center",
          duration: 2000,

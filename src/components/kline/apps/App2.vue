<template>
    <span>
        <div class="multi-chart">
            <chartbox v-for="(timeframe, id) in charts" :key="id" :id="id" :tf="id" v-bind:data="timeframe"
                :width="cbox_width" :height="cbox_height" :night="night" :ext="ext" :xsett="xsett" :resetkey="resetkey">
            </chartbox>
        </div>
        <div class="link-icon" :style="lst" @click="onlink">
        </div>
        <codepane v-if="code" :colors="colors" :width="500" :height="500" :src="rules" @src-changed="src_changed"
